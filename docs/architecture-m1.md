...
| `SET_CLOSURE` | strtable 索引 | 弹栈顶捕获值 → **clone 进栈下函数对象（`peek` 栈下 1 位，调用约定：定义点先 `LOAD_FUNCTION` 压函数值）的 `closure_scope` 捕获槽**——scope 层 define-or-replace（`scope_set`：占位 undefined 先销毁替换）。闭包为 clone 值语义（捕获独立副本，非引用）。hoist 提升区先 `PUSH_UNDEFINED` 占位、定义点 `SET_CLOSURE` 替换真实值——提升后、定义点前调用读到占位 undefined（TDZ 语义） | `scope_set` |
...
- **闭包捕获绑定（显式捕获列表）**：有捕获的函数，hoist 注册段在 `SET_FUNC_NAME` 之后为**每捕获**发 `PUSH_UNDEFINED; SET_CLOSURE "name"` 占位（函数值留在栈顶，`SET_CLOSURE` peek 栈下 1 位）——捕获槽先以 undefined 绑定，规避函数提升后、定义点前调用读到未初始化槽（TDZ 语义）。函数**定义点**发绑定序列 `LOAD_FUNCTION <fid>; <捕获值>; SET_CLOSURE "name"; ...; [POP]`：纯 id 捕获 → `PUSH "name"`（当前作用域链查外层变量值，借用引用）；括号 VALUE DECL `(b:i32 = c+d)` → 编译期求值 `init` 构造临时捕获值。`SET_CLOSURE` 把捕获值 **clone** 进函数 `closure_scope`（`scope_set` define-or-replace：占位 undefined 先销毁替换）。语句级局部函数定义点序列尾 `POP` 丢弃函数值（名字绑定已由提升完成，净 0）；函数字面量（表达式）不 POP——函数值留栈顶作表达式结果。闭包不参与类型：签名擦除捕获列表，`func(i32)->i32` 与 `func |x| ...:func(i32)->i32` 同一类型。
