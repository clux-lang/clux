...
| `LOAD_FUNCTION` | 函数 id | 从 `vm->functions_by_id` 按 id 查函数并压其 func value（type = `fn->type` 签名，data = `func_t`）——运行期函数值加载路径（与 `LOAD_TYPE` 对称：类型值/函数值都是编译期折叠 + 运行期查表加载）。内建函数（printf）创建时预登记 id 0；程序函数 id ≥64 由 `BIND_FUNC` 登记 | `vm_func_load` |
...
| `MAKE_FUNCTION` | 函数 id | 从 `vm->functions_by_id` 拉取**基底**函数对象 → **`func_instantiate` 实例化独立新实例**（新 `bcode_function_t`：共享基底 entry_pc/cfunc/type/id/name、`owns_name=false`；新 `closure_scope` 按基底捕获槽名以 undefined 占位）→ func value 压栈。函数定义（字面量/局部函数）每次求值生成新对象——循环内多个闭包捕获互不干扰（对标 `LOAD_FUNCTION` 的"同一对象"引用语义）。hoist 提升区构造的基底仅绑定地址；全局函数无捕获，基底即最终实例 | `func_instantiate` |
| `SET_CLOSURE` | strtable 索引 | 弹栈顶捕获值 → **clone 进栈下函数对象（`peek` 栈下 1 位，调用约定：定义点先 `MAKE_FUNCTION` 压函数值）的 `closure_scope` 捕获槽**——scope 层 define-or-replace（`scope_set`：占位 undefined 先销毁替换）。闭包为 clone 值语义（捕获独立副本，非引用）。定义点前的实例捕获槽为占位 undefined（TDZ 语义） | `scope_set` |
...
- **闭包捕获绑定（显式捕获列表）**：函数定义点发绑定序列 `MAKE_FUNCTION <fid>; <捕获值>; SET_CLOSURE "name"; ...; [STORE name; POP]`：`MAKE_FUNCTION` 从 `vm->functions_by_id` 拉取 hoist 区构造的**基底**函数对象，`func_instantiate` 实例化独立新实例（共享 entry_pc/cfunc/type/id/name，新 closure_scope 按基底捕获槽名以 undefined 占位）；纯 id 捕获 → `PUSH "name"`（当前作用域链查外层变量值，借用引用）；括号 VALUE DECL `(b:i32 = c+d)` → 编译期求值 `init` 构造临时捕获值。`SET_CLOSURE` 把捕获值 **clone** 进实例 `closure_scope`（`scope_set` define-or-replace：占位 undefined 先销毁替换）。语句级局部函数定义点序列尾 `STORE "name"` 把新实例重定向到作用域绑定（`value_assign` 改写 data 指针）+ `POP` 丢弃结果（函数值返回的 dst 引用，净 0）；函数字面量（表达式）keep=true 不 STORE——函数值留栈顶作表达式结果。**函数定义每次求值生成独立新实例**——循环内多个闭包捕获互不干扰。闭包不参与类型：签名擦除捕获列表，`func(i32)->i32` 与 `func |x| ...:func(i32)->i32` 同一类型。
